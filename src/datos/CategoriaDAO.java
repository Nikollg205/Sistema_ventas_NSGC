...
public class CategoriaDAO implements CrudSimpleInterface<Categoria> {
    ...
    public boolean desactivar(int id) {
        resp= false;
        try {
            ps=CON.conectar().prepareStatement("UPDATE categoria SET activo=0, descripcion=? WHERE id=?");
            ps.setInt(1, id);
            if(ps.executeUpdate()>0){
                resp = true;
            }
    ...
    public boolean activar(int id) {
         resp= false;
        try {
            ps=CON.conectar().prepareStatement("UPDATE categoria SET activo=1, descripcion=? WHERE id=?");
            ps.setInt(1, id);
            if(ps.executeUpdate()>0){
                resp = true;
            }
